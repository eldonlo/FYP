...
def noniid_partition_loader(
    data, bsz=10, m_per_shard=25, n_shards_per_client=2
):
    ...
    client_data = [
        torch.utils.data.TensorDataset(
            torch.cat([img[shards_idx[j]] for j in range(i * n_shards_per_client, (i + 1) * n_shards_per_client)]),
            torch.cat([label[shards_idx[j]] for j in range(i * n_shards_per_client, (i + 1) * n_shards_per_client)])
        )
